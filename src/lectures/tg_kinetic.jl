...
# ╔═╡ 77106420-01b4-11f1-afa5-3fc26aec170b
begin
...
# ╔═╡ e3b752b3-de45-434d-8369-c2c6ff171f87
md"""
импорт данных ТГ
"""

# ╔═╡ 0cc812a2-70c1-43aa-88cc-09c0b758c17d
begin
	#Парсер (сырые данные)
	struct TGParser
		time
	...
	#Структура данных ТГ со степенью превращения итд
	struct TGDataFrame
		time
		temperature
		mass
		...
		function TGDataFrame(time, temperature, mass)
			α = get_α(mass)
			dadt = get_dadt(α)
			new(time, temperature, mass, α, dadt)
		end

		function TGDataFrame(data::TGParser)
			
			α = get_α(data.mass)
			dadt = get_dadt(α)
			new(data.time, data.temperature, data.mass, α, dadt)
		end
	end

	function get_α(mass)
		return @. abs(mass[1]-mass)/(mass[1]-mass[end])
	end
	...
	function tg_interpolation(data::TGParser, method)
...
# ╔═╡ 5d385f07-da44-4539-ae24-3266860d04e6
md"""
путь до файла
"""

# ╔═╡ b04696ce-5d69-4fb4-9964-8ee05c05bd93
filedir = "D:\\julia_projects\\ThermoKineticjl\\test_data\\FN_KINETIC_2025\\fn_600_1_tg.txt";

# ╔═╡ 5385ff2c-d6f6-4d86-9801-ebc11fc755f3
raw_data = TGParser(filedir, 35)

# ╔═╡ a3fe9368-848d-4392-93cd-52427a6c1534
df = (TGDataFrame(raw_data))

# ╔═╡ c22a3fb9-6336-46df-aec0-a07acdf5d7e8
md"""
Попробуем решить диффуру
"""

# ╔═╡ 7fd3da2a-07fd-4ef3-bca4-16ff73f87695
const R = 8.314;

# ╔═╡ 5084c678-8bb1-4fc4-a552-a1e739b276db
begin
	function K(A, Ea, T)
		return A*exp(-Ea/(R*T))
	end
	function f_model(α, params)
		n = params
		return (1-α)^n
	end
	u₀ = 1e-2;
	...
	Foll = 0.5;
	tspan_kin = (0,1000)
	params_kin = [Ea1,Ea2,A1, A2, T, n1, n2]

	function fₚ(u,p,t)
		α = u
		Ea1, Ea2, A1, A2, T, n1, n2 = p;
		return Foll*K(A1, Ea1, t)*f_model(α, n1)+(1-Foll)*K(A2, Ea2, t)*f_model(α, n2)
		
	end
	
end

# ╔═╡ feb7d995-dc95-49ce-a34d-90ea5595e803
begin
	prob = ODEProblem(fₚ, u₀,tspan_kin, params_kin)
end

# ╔═╡ 842334f2-be18-4ae6-860f-17fc8f63ba06
function loss_function()
	
end

# ╔═╡ aed890a8-65ff-4864-986f-ab28976ec1a0
sol = solve(prob)

# ╔═╡ 132c4db1-3ce0-470d-9519-4b307e3084ef
function residuals(df::TGDataFrame, sol)
	df.α-sol(df.time)
end

# ╔═╡ 134e9228-53eb-4bb3-86b2-bbbdf0f2bf05
begin
	plot(residuals(df, sol))
	plot!(sol)
	plot!(df.time, df.α)
end

# ╔═╡ e9b754a3-548d-4550-9d7d-c6269c4fe8fb
function loss_f(df::TGDataFrame, sol)
	sum(abs2,residuals(df, sol))
end

# ╔═╡ dcf4e6df-79f9-4648-ab51-e86a21dacfa9
loss_f(df,sol)

# ╔═╡ 67babf84-f171-4344-a838-7993d85ab959
function loss(params_kin)
    prob = ODEProblem(fₚ, u₀,tspan_kin, params_kin)  
    sol = solve(prob)
    return sum(abs2,residuals(df, sol))
end

# ╔═╡ 28a4dd12-d258-4f16-b161-f3e32ad2ba6c
res = optimize(loss, params_kin)
...
manifest_format = "2.0"
project_hash = "0e2a967bea622482f368ecd060f13c84b43f4112"
...
git-tree-sha1 = "df050402e2db17f534c97032be5a907bfc8fbf7d"
