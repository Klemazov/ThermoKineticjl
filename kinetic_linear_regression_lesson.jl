...
# ╔═╡ 4d40f2a0-0b4f-11f1-afc2-794be6f4c1af
begin
	using PlutoUI;
	using Plots;
	using CSV;
...
# ╔═╡ fdde6926-9ff4-4a09-8d28-c67a8abe9715
PlutoUI.TableOfContents()

# ╔═╡ 803540ae-9b04-4ee0-98dc-2fd7b811c435
md"""
По большей части тема семинара это Рекомендации Комитета по кинетике ICTAC по проведению кинетических вычислений на основе данных термического анализа.

*ICTAC - Международная конфедерация термического анализа и калориметрии*

# Введение
Под кинетическим процессом будем понимать процесс, протекающий во времени. В рамках термического анализа можно говорить о том, что протекают какие-то процессы во времени при определенной температуре. Основная задача такие процессы как-то описать. 

Основное уравнение, которое используется для описания таких процессов можно записать следующим образом:

$\frac{d\alpha}{dt} = K(T)f(\alpha)$

Температурную зависимость $K(T)$ описывают из следующих предположений. Химическая реакция может идти только при столкновении молекул. Молекулы должны обладать некоторой энергией $Ea$, чтобы они начали взаимодействовать между собой. Известно, что число молекул, обладающих энергией ${E>E_{a}}$, пропорционально экспоненте:

$K(T)∼exp(-Ea/RT)$

Вероятность столкновения также определяется также тем насколько они правильно расположены между собой (то есть не все молекулы даже с нужной энергией учавствуют в столкновении). 

При $T\rightarrow \infty$ $exp(-Ea/RT)\rightarrow exp(0) = 1$

При бесконечно большой температуре все молекулы должны учавствовать в реакции. Это и выражает предэкспоненциальный множитель (Константа скорости реакции при бесконечно большой температуре) 

Итого имеем следующее уравнение для K:

$K(T) = A exp(-Ea/RT)$

Функция $f(\alpha)$ описывает тип реакции 

Также в общем случае скорость реакции зависит от давления:

$\frac{d\alpha}{dt} = K(T)f(\alpha)h(P)$


Однако для наших процессов зачастую можно считать, что зависимости нет и конечное уравнение можно ограничить двумя множителями

Также нужно сказать, что Аррениусовская зависимость не всегда физически применима к некоторым процессам, так как вывод уравнения получен для газов
"""

# ╔═╡ bd2405f6-ca69-45e4-a1cd-22f1766fca99
md"""
# Типы реакций

Под типом реакции я буду понимать вид функции $f(\alpha)$ 

Для различных реакций вид функции будет различный. 

Типичные примеры:

| тип реакции |  $f(\alpha)$ | дополнительные параметры|поведение функции|
| --- | --- |--- |--- |
| реакция первого порядка | $1-\alpha$ |-|затухающая|
| реакция второго порядка| $(1-\alpha)^2$ |-|затухающая|
| реакция n-ого порядка| $(1-\alpha)^n$ |n|затухающая|
|степенная реакция|$n\alpha^{n-1/n}$|n|ускоряющаяся|
|Модель Авраами-Ерофеева|$n(1-\alpha)[-ln(1-\alpha)^{n-1/n}]$|n|сигмоида|
|Модель Шестака-Бергрена|$\alpha^m(1-\alpha)^n[-ln(1-\alpha)^p]$|n,m,p|сигмоида|
|Расширенное уравнение  Праута-Томпкинса|$\alpha^m(1-\alpha)^n$|n,m|сигмоида|

Посмотрим как они выглядят визуально

Вид функции будет соответствовать "изотермическому" режиму при единичных значениях энергии активации и предэкспоненциального множителя 
"""

# ╔═╡ 690dc85a-38be-4fba-a581-51d118bab8dc
begin
	f1(α) = 1-α
	f2(α) = (1-α)^2
	fn(α,n) = (1-α)^n
...
# ╔═╡ 7c4b6892-acb9-4d35-b3c4-bda5e666ffd5
md"""
n = $@bind nₛ Slider(0.1:0.1:10.0; default=1, show_value=true)

m = $@bind mₛ Slider(0.1:0.1:10.0; default=1, show_value=true)

p = $@bind pₛ Slider(0.1:0.1:10.0; default=1, show_value=true)
"""

# ╔═╡ fdf260b9-7a06-4162-8403-047effc12fac
begin
	αₛ = range(start = 0.1, stop = 0.9, length = 100);
	
	plotfn = plot(αₛ, (@. fn(αₛ,nₛ)), label = "реакция n-ого порядка")
	plotpn = plot(αₛ, (@. pn(αₛ,nₛ)), label = "степенная реакция")
	plotaf = plot(αₛ, (@. af(αₛ,nₛ)), label = "Авраами-Ерофеев")
	plotsb = plot(αₛ, (@. sb(αₛ,nₛ, mₛ, pₛ)), label = "Шестак-Бергрен")
	plotpt = plot(αₛ, (@. pt(αₛ,nₛ, mₛ)), label = "Праут-Томпкинс")
	plot(plotfn, plotpn, plotaf, plotsb, plotpt)
end

# ╔═╡ 47127698-aed4-48ea-8292-a9bb5402d3f9
md"""
Мы посмотрели как выглядят различные $f(\alpha)$

попробуем сгенерировать то как будут выглядеть экспериментальные данные $\alpha$ с различными типами реакций. 

"режим нагрева" будет линейным, то есть $T(t)=\beta t$ 

Параметры $Ea$ и $A$ и скорость сделаем равными 1

получим следующее уравнение

$\frac{d\alpha}{dt} = exp(\frac{-1}{t})f(\alpha)$

по сути это дифференциальное уравнение. В качестве начального условия положим, что 

$\alpha(0) = 0$
"""

# ╔═╡ 39279f76-eef5-4ed9-a959-1274eb33e016
# begin
# 	const R = 8.31;
# 	tspanₛ = (0.0,1.0)
# 	α₀ = [0.0]
...
# 	Aₛ = 1.0;
# 	T(t,β) = β*T; 
# 	K(t,Ea,A,T) = A*exp(-Ea/(R*T))
# 	paramsₛ = [Eaₛ, Aₛ, βₛ, [nₛ]]
# 	function kinetic_equation(t,Ea,A,β,α, f)
# 		K(t,Ea,A,T(t,β))*f(α)
# 	end
# 	function kinetic_equation(t,Ea,A,β,α,f,fparams)
# 		K(t,Ea,A,T(t,β))*f(α,fparams...)
# 	end

# 	function odefunc(u,p,t)
# 		α = u
# 		Ea, A, β, fparams = p
# 		kinetic_equation(t,Ea,A,β,α,fn,fparams)
# 	end

	

	
# 	probₛ = ODEProblem(odefunc, α₀, tspanₛ, paramsₛ)
# 	solₛ = solve(probₛ)
# end
...
manifest_format = "2.0"
project_hash = "88f8e09df1f19cb1e02022cde3088c4ce8b19f40"
